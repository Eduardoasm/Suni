export function getFirstAndLastDayOfWeekOfMonth(startDate: Date) {
  const year = new Date(startDate).getFullYear();
  const month = new Date(startDate).getMonth() + 1;
  const firstDayOfMonth = new Date(year, month - 1, 1);
  const firstDayOfWeekOfMonth = new Date(firstDayOfMonth);
  firstDayOfWeekOfMonth.setDate(
  ...
  lastDayOfWeekOfMonth.setDate(
    lastDayOfMonth.getDate() + (6 - lastDayOfMonth.getDay() + 1)
  );
